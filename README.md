...
- **足迹统计**：自动统计去过的大洲、国家和地区、省 / 州、市、县 / 区、镇 / 乡，以及去过的**世界遗产**（共 1263 处）和 **TCC 名单**（旅行家世纪俱乐部的 330 个国家和地区）。点任意一项，地图就跳到对应的标注。
...
- **世界年表**（工具箱里）：中国、埃及、两河流域与波斯、印度、希腊、罗马、西欧、法国、日本、美洲、非洲十一个文明的编年横向对照，中国下面另有西域、青藏高原、东北、蒙古高原四行边疆地区的政权；中国各朝代标有都城（含今址）和开国者。中国一行固定在最上面作参照，移动光标就能看到"这一年中国是什么朝代"；点任意时期可以看到同时期的其他文明。中文、English、Français 可切换，也可以三语对照显示。另有 414 条大事记（中国 157 条、其他文明和世界大事 257 条），在各行底部以圆点标出，点开可以看到同一年的中国朝代，朝代详情里也会列出这一时期的大事。时期数据在 `public/data/chronology.json`，大事记在 `public/data/chronology-events.json`，都可以自己增补。
...
- **导出旅程记录**：把所有标注和运动轨迹按时间顺序整理成一份 Markdown 文字，包括时间、坐标、国家 / 省 / 市 / 县 / 镇、世界遗产、所属旅程和描述，轨迹列出距离、用时、配速和爬升。适合存档、打印或贴进笔记软件。
...
- 和上面的"国家和地区"数不是一回事：TCC 把一些大国里距离本土较远的部分单独计数（夏威夷、西西里、苏格兰、桑给巴尔、西藏……）。
- 330 条里 327 条能自动匹配：先看标注属于哪个国家 / 地区，再用省 / 州名或经纬度框分出是哪一条（比如冲绳算「琉球群岛」、拉萨算「西藏」、巴厘岛算「小巽他群岛」）。
- 剩下 3 条是南极洲互相重叠的领地声索（阿根廷、英国、智利都声索南极半岛），没法可靠区分，不参与匹配，面板里有说明。数字只会偏小，不会虚高。
- 完整名单在工具箱的「数据统计」里，见下面一节。
- 细分用的经纬度框是矩形近似，海岸线附近或相邻岛群的边界上可能判错。判错时改标注的「所在地区」没有用（规则看的是坐标），可以直接改 `server/data/tcc-regions.json` 里的 `when` 规则，重启服务生效。
- 按 TCC 的规矩，哪怕只是中途落地下机踏上过当地土地也算去过。
...
工具箱里的 **数据统计** 是三个名录页，都是中英法三语，去过的地方用琥珀色突出，没去过的是灰的：

| 页面 | 内容 |
|---|---|
| 国家和地区（`/pages/countries.html`） | ISO 3166-1 的 248 个国家和地区，加上规则里单独分出来的地区，共 250 个。**带国旗**，按大洲分组 |
| TCC 名单（`/pages/tcc.html`） | 旅行家世纪俱乐部的 330 个国家和地区，按 TCC 自己的 12 个分区分组 |
| 世界遗产（`/pages/heritage.html`） | 全部 1263 处，按国家分组，跨国的归到「多国共有」，点名称打开 UNESCO 官网的介绍 |

三个页面共用一套外壳：顶部是「已去过 x / 总数」和进度条，可以按名称搜索（**三种语言都能搜**，中文界面下输入英文或法文名也找得到），也可以只看已去过或还没去的。页面之间用顶部的标签切换，语言选择和工具箱共用。

国旗是 [flag-icons](https://github.com/lipis/flag-icons)（MIT 协议）的 SVG，存在 `public/vendor/flags/`，不联网。单独分出来的地区（北塞浦路斯、索马里兰）没有对应的旗帜文件，显示为空位。

### 特殊地区

有些地方被 OpenStreetMap 归入了某个国家，但你想把它单独统计。这类规则写在 [`config/special-regions.json`](config/special-regions.json) 里，自带 15 条：
...
| 法国的 12 个海外属地 | 国家代码是 `fr`，并且落在该属地的经纬度范围内 |

法国的海外属地（留尼汪、瓜德罗普、马提尼克、法属圭亚那、马约特、圣皮埃尔和密克隆、圣巴泰勒米、法属圣马丁、法属波利尼西亚、新喀里多尼亚、瓦利斯和富图纳、法属南部领地）在 OSM 里的国家代码一律是 `fr`，不分出来的话会被并进法国，国家数和 TCC 名单都少算。它们各自的 ISO 代码（`re`、`gp` 等）就是规则里的 `code`。
...
└── public/                 # 前端（ES 模块，无需构建）
    ...
    ├── lang/               # zh、en、fr
    ├── data/travelers/  data/trails/  data/chronology.json
    ├── pages/              # 工具箱：tools.html；数据统计三页 countries / tcc / heritage.html（共用 lists.css + lists.js）；
    │                       #   chronology.html（世界年表）、imgexif.html
    ├── img/
...
| GET | `/api/stats` | 足迹统计：各级行政区、世界遗产、TCC 名单、后台查询进度 |
| GET | `/api/lists/countries` | 全部国家和地区（代码、大洲、有没有国旗）加上你去过哪些 |
| GET | `/api/lists/tcc` | TCC 完整名单（330 条三语名称）加上你去过哪些 |
| GET | `/api/lists/heritage` | 世界遗产完整名录（1263 处）加上你去过哪些 |
