// 足迹统计面板：顶部是各级数量，中间是"大洲 → 国家 → 省 → 市 → 县 → 镇"的层级树，最后是世界遗产
...
const unescoUrl = (id) => `https://whc.unesco.org/${UNESCO_LANGS[getLanguage()] || 'en'}/list/${id ? id + '/' : ''}`;
...
function heritageHtml(heritage) {
    ...
    return `<details class="stat-section" data-section="heritage" ${openSections.has('heritage') ? 'open' : ''}>
        ...
        <p class="stat-all-link"><a href="${escapeHtml(unescoUrl())}" target="_blank" rel="noopener">${escapeHtml(t('heritageAll'))}<svg><use href="#i-external"/></svg></a></p>
    </details>`;
}
...
function render() {
    const { totals, counts, tree, heritage } = data;
    nodeIndex = new Map();

    // 一排七个小方块：方块上用短名称，完整名称和总数放在悬停提示里
